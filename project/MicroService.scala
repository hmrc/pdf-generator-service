  ...
  lazy val microservice = Project(appName, file("."))
    ...
    .settings(
      downloadBinaryDependenciesTask := {
        ...
        s"tar zxf ${ghostscript.absolutePath.replaceAll("\\s", "%20")} -C ${binDir.getAbsolutePath.replaceAll("\\s", "%20")} --strip-components 1".!
        s"chmod +x ${binDir.getAbsolutePath.replaceAll("\\s", "%20")}/gs-920-linux_x86_64".!
        ghostscript.delete()
        ...
        s"tar xJf ${wkhtmltox.absolutePath.replaceAll("\\s", "%20")} -C ${extraDir.getAbsolutePath.replaceAll("\\s", "%20")} --strip-components 1".!
        s"chmod +x ${binDir.getAbsolutePath.replaceAll("\\s", "%20")}/wkhtmltopdf".!
        wkhtmltox.delete()
      },
