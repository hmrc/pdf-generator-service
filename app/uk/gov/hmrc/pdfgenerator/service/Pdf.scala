...
import uk.gov.hmrc.pdfgenerator.utils.{DestinationDocumentLike, NoExecutableException, PdfConfig, SourceDocumentLike}
...
class Pdf(executablePath: String, config: PdfConfig) {
  ...
  /**
    ...
    */
  def run[A, B](sourceDocument: A, destinationDocument: B)(
    implicit sourceDocumentLike: SourceDocumentLike[A],
    destinationDocumentLike: DestinationDocumentLike[B]): Int = {
